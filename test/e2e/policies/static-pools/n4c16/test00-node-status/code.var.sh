...
vm-wait-until 'kubectl get nodes -o jsonpath="{.items[*].metadata.labels}" | grep \"cmk.intel.com/cmk-node\"\:\"true\"' ||
    error "cmk.intel.com/cmk-node label missing"

out ""
out "### Verifying that amount exclusive cores on node matches /etc/cmk/pools.conf"
vm-wait-until 'kubectl get nodes -o jsonpath="{.items[*].status.allocatable}" | grep -q \"cmk.intel.com/exclusive-cores\"\:\"3\"' ||
