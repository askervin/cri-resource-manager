...
source-source-files() {
    ...
    # the real test code. This happens by prepending "source" commands
    # to the code.
...
root_dir_filename="run_tests_suite.md"
if compgen -G "$TESTS_DIR/$root_dir_filename" >/dev/null; then
    TESTS_ROOT_DIR="$TESTS_DIR"
elif compgen -G "$TESTS_DIR/../$root_dir_filename" >/dev/null; then
